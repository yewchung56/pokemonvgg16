...
class PreProcess:
    def __init__(self, file_path):
        ...
        self.userId_encoder = LabelEncoder()  # title_encoder 속성 추가
    ...
    def preprocess_data(self, df, sparse_features=['userId', 'title']):
        ...
        self.userId_encoder.fit(df['userId']) 
        df['userId_encoded'] = self.userId_encoder.fit_transform(df['userId'])

        return df, sparse_features
    def get_title_encoder(self):
        return self.title_encoder  # title_encoder 반환 메소드
    def get_userId_encoder(self):
        return self.userId_encoder  # title_encoder 반환 메소드
